...
class GridPooling(PointModule):
    ...
    def pca_csr_no_centered(self, points_sorted: torch.Tensor,
                            idx_ptr: torch.Tensor):
        """
        points_sorted: (N,3)  -- 클러스터 순 정렬
        idx_ptr      : (M+1,) -- 세그먼트 포인터
        return:
        R: (M,3,3)         -- 클러스터별 회전행렬
        mu: (M,3)          -- 클러스터별 평균
        cov: (M,3,3)       -- (옵션) 클러스터별 공분산 (필요하면)
        """

        # 1) sum_x : (M,3)
        sum_x = torch_scatter.segment_csr(points_sorted, idx_ptr, reduce="sum")
        counts = idx_ptr[1:] - idx_ptr[:-1]  # (M,)
        mu = sum_x / counts.unsqueeze(-1).clamp_min(1e-9)  # (M,3)

        # 2) sum_xx: (M,3,3)
        #   각 점에 대해 x_i x_i^T (3x3) 만들어 세그먼트별 sum
        #   => shape(N,9) => segment_csr => shape(M,9) => reshape(M,3,3)
        outer_9 = (points_sorted.unsqueeze(2)*points_sorted.unsqueeze(1)) # (N,3,3)
        outer_9 = outer_9.reshape(-1,9)                                   # (N,9)
        sum_xx = torch_scatter.segment_csr(outer_9, idx_ptr, reduce="sum")             # (M,9)
        sum_xx = sum_xx.reshape(-1,3,3)                                  # (M,3,3)

        # 3) E[xx^T] = sum_xx / counts
        E_xxT = sum_xx / counts.view(-1,1,1).clamp_min(1e-9) # (M,3,3)

        # 4) Cov = E[xx^T] - mu mu^T
        #   => shape(M,3,3)
        #   => PCA via SVD => R
        mu_expand = mu.unsqueeze(-1)  # (M,3,1)
        mu_muT = torch.matmul(mu_expand, mu_expand.transpose(-1,-2)) # (M,3,3)
        Cov = E_xxT - mu_muT
    ...
    def canonical_transform(
        ...
        """
        points: (N,3) 원래 좌표 (정렬X)
        indices: (N,) 정렬인덱스
        idx_ptr: (M+1,)
        R: (M,3,3)
        mu: (M,3)
        return canonical: (N,3)
        """
        # 1) points_sorted
        points_sorted = points[indices]  # (N,3)
        # 2) expand R, mu => repeat_interleave
        repeats = (idx_ptr[1:] - idx_ptr[:-1]).tolist()  # (M,)
        R_expanded = torch.repeat_interleave(R, repeats, dim=0)   # (N,3,3)
        mu_expanded = torch.repeat_interleave(mu, repeats, dim=0) # (N,3)

        # 3) canonical_sorted = (points_sorted - mu_expanded) @ R_expanded^T
        centered = points_sorted - mu_expanded
        canonical_sorted = torch.einsum(
            'nc,ncd->nd', centered, R_expanded.transpose(-1,-2)
        )  # (N,3)

        return canonical_sorted


    def scatter_content_orientation_csr(self, points, indices, idx_ptr):
        """
        points : (N,3) un-sorted
        indices: (N,) cluster-sorted index
        idx_ptr: (M+1,) CSR pointer
        returns:
        canonical_sorted: (N,3) indices-sorted 값. 나중에 csr maxpooling 해야하기 때문
        R: (M,3,3)
        mu: (M,3)
        """
        # 1) points_sorted
        ...
        # 2) PCA => R, mu
        R, mu, Cov = self.pca_csr_no_centered(points_sorted, idx_ptr)

        # 3) canonical
        canonical_sorted = self.canonical_transform(points, indices, idx_ptr, R, mu)
        return canonical_sorted, R, mu
    ...
    def forward(self, point: Point):
        ...
        canonical_sorted, R, mean_xyz = self.scatter_content_orientation_csr(point.coord, indices, idx_ptr) 
        canonical_sorted = self.proj(canonical_sorted)
        
        point_dict = Dict(
            feat=torch_scatter.segment_csr(canonical_sorted, idx_ptr, reduce=self.reduce) + self.proj_rot(R.reshape(-1, 9)),
